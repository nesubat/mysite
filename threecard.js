"use strict";
const MINP = 2, MAXP = 17, KEY = "threecard:v1";
...
/* ---------- scoring ----------
   One winner a round. The winner takes a point off every other player, so the
   winner gets +(n-1) and everyone else -1, and the round sums to zero. Same as
   a plain dhumbal win, just with a bigger table.                              */
function scoreRound(n, winner){
  const s = [];
...
function freshDraft(){ return { winner: null }; }
function roundNo(){ return S.editing !== null ? S.editing + 1 : S.rounds.length + 1; }
function draftReady(){ return !!S.draft && S.draft.winner !== null; }
...
function viewSetup(){
  ...
  return '<div class="card setup">' +
    ...
    '<p class="sub">Two to seventeen at the table. You can rename them later.</p>' +
    '<div class="counter">' +
...
/* ---------- the round ---------- */
function seatButtons(selected){
  let out = '<div class="seats">';
  for (let i = 0; i < S.names.length; i++){
    out += '<button class="seat' + (selected === i ? " pick" : "") + '" ' +
      'data-act="pick" data-i="' + i + '">' + esc(S.names[i]) + '</button>';
  }
  return out + '</div>';
}
...
function roundPreview(){
  ...
  const s = scoreRound(n, d.winner);
  let rows = "";
  for (let i = 0; i < n; i++){
    rows += '<div class="prow"><div class="who">' + esc(S.names[i]) +
      (i === d.winner ? "<i>won</i>" : "") + '</div>' +
...
function viewRound(){
  ...
  return '<div class="card">' +
    ...
    '<p class="sub">Only one player wins. Everyone else drops a point.</p>' +
    '<p class="seclabel">Who won?</p>' +
    seatButtons(S.draft.winner) +
    roundPreview() +
...
function padFor(title, names, rounds, live){
  ...
  for (let r = 0; r < rounds.length; r++){
    ...
    for (let i = 0; i < n; i++){
      ...
      cells += '<td class="' + (v < 0 ? "neg" : "") + '">' + plus(v) + '</td>';
    }
  ...
  return '<div class="pad"><h2>' + title + '</h2><div class="padscroll"><table>' +
    '<thead><tr><th class="rlabel"></th>' + names.map(x => '<th>' + esc(x) + '</th>').join("") + '</tr></thead>' +
    '<tbody>' + body + '</tbody>' +
    ...
    (live ? '<p class="padnote">' + (rounds.length
      ? "Tap a round number to correct it. Scroll sideways for the rest of the table."
  ...
  if (!h.length){
    ...
    list = '<ul class="hist">' + h.map(m => {
      ...
      return '<li><button data-act="open" data-id="' + m.id + '">' +
        ...
        m.names.length + ' players</span></span>' +
...
app.addEventListener("click", function(e){
  ...
  if (act === "save"){
    if (!draftReady()) return;
    const entry = { winner: d.winner, scores: scoreRound(S.names.length, d.winner) };
  ...
  if (act === "reopen"){ S.phase = "round"; S.draft = freshDraft(); return render(true); }

  if (act === "edit"){
    const r = +el.dataset.r, old = S.rounds[r];
    S.editing = r;
    S.draft = { winner: old.winner };
    S.phase = "round";
    window.scrollTo({ top: 0, behavior: "smooth" });
    return render(true);
  }
  ...
  if (act === "cancel-edit"){
    S.editing = null; S.draft = freshDraft(); S.phase = "round";
    return render(true);
  }
...
function exportBackup(){
  const blob = new Blob([JSON.stringify({ version:1, game:"threecard",
    exportedAt:new Date().toISOString(), history: store.history }, null, 2)],
    { type:"application/json" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = "three-card-history-" + new Date().toISOString().slice(0,10) + ".json";
